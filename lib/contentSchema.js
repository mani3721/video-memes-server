...
/** Cap for the single rich-text body format (HTML, so longer than plain text). */
 ...
 *
 * Accepts two formats:
 *   - New: { body: '<html string>' } — from the single rich-text editor
 *   - Legacy: { what, why, how, quality, related } — keyed plain-text sections
 *
 * Unknown keys are dropped rather than rejected so an older client build
 * cannot cause a save to fail by omitting a key it has never heard of.
 *
...
export function parseDescriptionLong(input) {
  ...
  // New rich-text format: single HTML body key
  if ('body' in input) {
    const raw = input.body
    if (typeof raw !== 'string') return { error: 'description_long.body must be a string.' }
    const body = raw.trim()
    if (body.length > MAX_BODY_CHARS) {
      return { error: `description_long.body must be ${MAX_BODY_CHARS} characters or fewer.` }
    }
    return { value: body ? { body } : null }
  }
...
export function parseContentPatch(body) {
  ...
  if ('is_hot' in body) {
    patch.is_hot = Boolean(body.is_hot)
  }
