...
// ── Source dispatch ──────────────────────────────────────────────────────────
// Groups read from one of two tables. Keeping the branch in three small
// functions means buildContentSitemap and buildIndexSitemap stay unaware of
// which table a group came from.

function countFor(config) {
  return config.source === 'blog' ? countBlogPosts() : countIndexable(config.categories)
}

function latestFor(config) {
  return config.source === 'blog' ? latestBlogChangeAt() : latestChangeAt(config.categories)
}

function fetchFor(config, offset, limit) {
  return config.source === 'blog'
    ? fetchBlogPage(offset, limit)
    : fetchIndexablePage(config.categories, offset, limit)
}
...
/**
 * The description Google shows alongside a video result.
 *
 * Prefers the authored "what is this meme" subsection, falling back to the
 * spec-derived sentence. Kept in step with describe() in
 * client/src/utils/seo.js so the sitemap and the page do not disagree about
 * what the asset is.
 */
function describe(row, maxChars = 320) {
  const authored = row.description_long?.what
...
/**
 * Blog post → sitemap URL entry.
 *
 * Editorial posts are long-form prose that is rewritten rarely, so they get a
 * flat weekly/0.6 rather than the download-driven tiers used for assets.
 */
function toBlogUrlEntry(post) {
  return {
...
export async function buildContentSitemap(group, page = 1) {
  ...
  const total = await countFor(config)
  const totalPages = pageCountFor(total)
  if (page < 1 || page > totalPages) return null

  const offset = (page - 1) * MAX_URLS_PER_FILE
  const rows = await fetchFor(config, offset, MAX_URLS_PER_FILE)

  const now = Date.now()
  const toEntry = config.source === 'blog' ? toBlogUrlEntry : (row) => toUrlEntry(row, config.media, now)
  return buildUrlSet(rows.map(toEntry))
}
...
export async function buildPagesSitemap() {
  ...
  // 'blog' is a pseudo-category: it names the blog_posts table rather than a
  // memes.category value, so it resolves through the blog query.
  const feedCategories = [...new Set(STATIC_PAGES.flatMap((p) => p.feedOf ?? []))]
  const latestByCategory = new Map(
    await Promise.all(
      feedCategories.map(async (category) => [
        category,
        category === 'blog' ? await latestBlogChangeAt() : await latestChangeAt([category]),
      ]),
    ),
  )
...
export async function buildIndexSitemap() {
  ...
  const groups = await Promise.all(
    GROUP_NAMES.map(async (group) => {
      const config = CONTENT_GROUPS[group]
      const [total, lastmod] = await Promise.all([countFor(config), latestFor(config)])
