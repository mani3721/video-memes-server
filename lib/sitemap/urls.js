 ...
 *
 * ⚠  Mirrors assetPathPrefix in client/src/utils/seo.js.
 *
 * Sounds sit under /sound/ because their list rows carry no download button —
 * the download only exists on the detail page, so that page must be a real
 * URL. Emitting /meme/ here while the client renders a /sound/ canonical would
 * make Google drop every sound URL as a duplicate.
 */
export function assetPathPrefix(meme) {
  return meme?.category === 'sounds' ? '/sound' : '/meme'
}
