 ...
 */
export const SITE_ORIGIN = (process.env.SITE_ORIGIN ?? 'https://videsaur.co.in').replace(/\/+$/, '')
 ...
 *   'none'  — plain <url>; there is no audio sitemap schema, so discovery for
 *             sounds relies on schema.org AudioObject on the page itself
 *             (see buildAudioSchema in client/src/utils/seo.js)
 */
export const CONTENT_GROUPS = {
  memes: { categories: ['videos'], media: 'video' },
  gifs: { categories: ['gifs'], media: 'image' },
  audio: { categories: ['sounds'], media: 'none' },
  templates: { categories: ['images'], media: 'image' },
}
